(function () {
	...
	function renderDialog() {
		...
		root.innerHTML = '<div class="npc-cinematic-dialog"><button type="button" class="quest-close npc-cinematic-dialog__close" data-action="close" title="ESC" aria-label="' + esc(tr("quest.common.close", "Zamknij")) + '">✕</button>' +
			'<section class="npc-cinematic-dialog__speech"><span class="npc-cinematic-dialog__name">' + esc(speakerName) + '</span><p>' + esc(speechText) + '</p>' + (isNode && !showOptions ? '<small>' + esc(tr("quest.dialog.advanceHint", "Spacja / LPM")) + '</small>' : '') + '</section>' +
