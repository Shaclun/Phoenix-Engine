...
local cookedMeat = fo(15, 99, "ITFOMUTTON.3DS", "Smażone mięso", { kind = "heal", data = { hp = 12 } })
local rawMeat = fo(5, 99, "ITFOMUTTONRAW.3DS", "Surowe mięso", { kind = "heal", data = { hp = 3 } })
phoenix.item.register("ITFOMUTTON", cookedMeat)
phoenix.item.register("ITFOMUTTONRAW", rawMeat)
// Legacy IDs remain readable for items obtained before canonical hunting loot was introduced.
phoenix.item.register("ITFO_MUTTON", cookedMeat)
phoenix.item.register("ITFO_MUTTONRAW", rawMeat)
